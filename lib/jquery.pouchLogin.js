...
(function($) {
    $.fn.couchLogin = function(opts) {
        var elem = $(this);
		console.log('login',elem,opts);
        opts = opts || {};
        function initWidget() {
			console.log('initwidget');
            $.couch.session({
                success : function(r) {
					console.log('got session',r);
        ...
        function doLogin(name, pass) {
			console.log('doLogin',name,pass);
            $.couch.login({name:name, password:pass, error:function(err) {console.log(err)},success:function(data) { console.log('loogin successa',data); initWidget() }});
        };
